...
**Target distribution:** pi(x) = p(x)^alpha where alpha > 1 concentrates on high-probability sequences.

**Proposal distribution:** q(x) = p(x) (base model at temperature=1)

**Metropolis-Hastings acceptance:**
```
log A = log(pi(x')/pi(x)) + log(q(x)/q(x'))
      = alpha*log p(x') - alpha*log p(x) + log p(x) - log p(x')
      = (alpha - 1) * [log p(x') - log p(x)]
```

For **alpha=1.67** (default): proposals with higher log probability are more likely to be accepted.

### What's Implemented

| Feature | Paper | `src/` | Notes |
|---------|-------|--------|-------|
| **Partial regeneration** | Yes | Yes | Pick random block boundary, regenerate suffix |
| **MH acceptance** | Yes | Yes | Accept/reject based on suffix log probs |
| **Block-wise generation** | B=192 tokens | Yes | Configurable via `mcmc.block_size` |
| **Restrict to last N blocks** | - | Yes | Cost optimization via `mcmc.restrict_to_last_n` |
